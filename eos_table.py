...
#
# GLOBAL VARIABLE
__version__ = '1.1.4' # 5/3/2023 added bilinear_ru table function and cs to EOSpoint
...
class EOSpoint:
    ...
    def __str__(self):
        ...
        return '\nClass EOSpoint: \n' + \
               ...
               f'   U, S, cs: {self.U} {self.S} {self.cs} \n' 
...
#
    ...
    def queryptlin(self,rho,t):
        ...
        pt.U = unew
        return pt
    def bilinear_ru(self,rho,u):
        """Bilinear interpolation for EOS state at input (rho,u)"""
        pt = EOSpoint()
        # linearly interpolate the table for return the rho,t point values
        rindex = np.where(self.rho >= rho)[0][0]
        tindex = np.where(self.U[:,rindex] > u)[0][0]
        P11 = self.P[tindex-1,rindex-1]
        P21 = self.P[tindex,rindex-1]
        P12 = self.P[tindex-1,rindex]
        P22 = self.P[tindex,rindex]
        ...
        U12 = self.U[tindex-1,rindex]
        U22 = self.U[tindex,rindex]
        S11 = self.S[tindex-1,rindex-1]
        S21 = self.S[tindex,rindex-1]
        S12 = self.S[tindex-1,rindex]
        S22 = self.S[tindex,rindex]
        C11 = self.cs[tindex-1,rindex-1]
        C21 = self.cs[tindex,rindex-1]
        C12 = self.cs[tindex-1,rindex]
        C22 = self.cs[tindex,rindex]
        r1 = self.rho[rindex-1]
        r2 = self.rho[rindex]
        t1 = self.T[tindex-1]
        t2 = self.T[tindex]
        u1 = U11 * (r2-rho)/(r2-r1) + U12 * (rho-r1)/(r2-r1)
        u2 = U21 * (r2-rho)/(r2-r1) + U22 * (rho-r1)/(r2-r1)
        s1 = S11 * (r2-rho)/(r2-r1) + S12 * (rho-r1)/(r2-r1)
        s2 = S21 * (r2-rho)/(r2-r1) + S22 * (rho-r1)/(r2-r1)
        p1 = P11 * (r2-rho)/(r2-r1) + P12 * (rho-r1)/(r2-r1)
        p2 = P21 * (r2-rho)/(r2-r1) + P22 * (rho-r1)/(r2-r1)
        c1 = C11 * (r2-rho)/(r2-r1) + C12 * (rho-r1)/(r2-r1)
        c2 = C21 * (r2-rho)/(r2-r1) + C22 * (rho-r1)/(r2-r1)
        p_int = p1 * (u2-u)/(u2-u1) + p2 * (u-u1)/(u2-u1)
        s_int = s1 * (u2-u)/(u2-u1) + s2 * (u-u1)/(u2-u1)
        t_int = t1 * (u2-u)/(u2-u1) + t2 * (u-u1)/(u2-u1)
        c_int = c1 * (u2-u)/(u2-u1) + c2 * (u-u1)/(u2-u1)
        pt.T = t_int
        pt.rho = rho
        pt.P = p_int
        pt.S = s_int
            ...
            if testout[i] == '  TWO-PHASE BOUNDARIES\n':
                ...
                while flag == 0:
                    if testout[j+i+4].find(' anphas') == 0:
                        ...
                        tmp4 = list(tmp3.split())
                        #print(tmp4)
#                        if (len(tmp4) >0) and (float(tmp4[3]) > 1.E-50) and (float(tmp4[4]) > 0): # stop if the pressures become negative on the vapor curve or very tiny
                        if (len(tmp4) >0) and (float(tmp4[3]) > 0) and (float(tmp[2])>1.e-20): # stop if the pressures become negative on the vapor curve or very tiny
